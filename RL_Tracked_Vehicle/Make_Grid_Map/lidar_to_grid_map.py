...
def generate_ray_casting_grid_map(ox, oy, xy_resolution, breshen=True):
    ...
    if breshen:
        for (x, y) in zip(ox, oy):
            ...
            if x == 0 and y == 0:
                occupancy_map[ix][iy] = -1  # occupied area 1.0
                occupancy_map[ix + 1][iy] = -1  # extend the occupied area
                occupancy_map[ix][iy + 1] = -1  # extend the occupied area
                occupancy_map[ix + 1][iy + 1] = -1  # extend the occupied area
            else:
